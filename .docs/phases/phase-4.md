...
| 5  | Ручное создание Заказа по звонку/WhatsApp                       | ✅ Завершён |
| 6  | Клиенты: список, поиск по телефону, карточка                    | ✅ Завершён |
| 7  | Категории и список Товаров                                      | ✅ Завершён |
| 8  | Форма Товара: Варианты, характеристики, категории               | ✅ Завершён |
| 9  | Фото Вариантов: загрузка, цвет, образец ткани, главное          | ✅ Завершён |
| 10 | Отчёт по продажам                                               | ✅ Завершён |
...
- [x] Созданный Заказ в БД: `status=confirmed`,
      ...
      приёмки: «сразу в „Подтверждён“, не „Новый“») — проверено
- [x] Без предоплаты / без позиций / доставка без адреса → форма не
      отправлена, поля подсвечены, значения сохранены — проверено (в
      т.ч. кириллица в значениях переживает перерисовку)
- [x] Двойная отправка → один Заказ (проверено: второй POST с уже
      использованным `manual_order_token` редиректит на форму без
      создания записи); сбой на `order_items` внутри `createOrder()` —
      тот же откат транзакции, что уже проверен в Тасках 1–4 для той же
      функции, повторно не гонялся
- [x] Без JS: телефон + «Найти» обычным GET перерисовывает форму с
      результатом (проверено); подмена `user_id` в обход поиска
      отклонена отдельной проверкой (`findUserById()` + `role=customer`)
...
**Definition of Done:**
- [x] Поиск по телефону находит и Покупателя с аккаунтом, и гостевые
      Заказы с этим телефоном (критерий приёмки `FR-ADM-002`) —
      проверено на реальной БД: `+70000000000` даёт и строку «Аккаунт»
      (Nik Ran), и строку «Гость» (3 заказа) — тот же телефон не связан
      `linkGuestOrdersToUser()` (email при регистрации был другой), и
      обе строки корректно не путаются
- [x] Гость с 3 Заказами — одна строка с «3 заказа» — проверено на
      реальных данных; Покупатель с ранее привязанными гостевыми
      Заказами — отдельно не воспроизводился (нет такого тестового
      случая в текущей БД), логика опирается на уже существующее и
      проверенное поведение `linkGuestOrdersToUser()` (обнуляет
      `guest_*`, что исключает Заказ из ветки Гостя в `UNION`)
- [x] `8 (918) …` и `+7918…` дают один результат (через
      `normalizePhone()`, тот же код, что везде в проекте) — код-ревью,
      отдельно не гонялось
- [x] Карточка гостя показывает только его Заказы; чужой/несуществующий
      `key` → 404 — проверено (`user/99999` и `guest/+79990000099`)
- [x] Список читаем на 320px — не проверено, нет браузера в сессии
      (тот же пробел, что в Тасках 1–5); весь вывод через `e()` —
      код-ревью
- [x] Проверить `.docs/dod-global.md`
...
- [x] `slugify('Диван «Модерн» 2-х местный')` → `divan-modern-2-h-mestnyy`
      ...
      пустой результат (вход из одних спецсимволов, включая случай,
      когда и `name`, и `slug` пустые) → ошибка формы, проверено на
      реальной БД
- [x] Родитель второго уровня недоступен в выборе; ручной POST с
      `parent_id` подкатегории (в т.ч. в обход `<select>`) → ошибка,
      строка не создана — проверено на реальной БД
- [x] Дубликат `slug` → отдельная ошибка поля («занят другой
      категорией», не путается со структурной «не удалось
      сформировать адрес»); категория с Товарами не удаляется
      (дружелюбное сообщение, проверено на «Диваны»); пустая —
      удаляется — проверено
- [x] Скрытый Товар пропадает из `/catalog` (`404` на `/product/{slug}`)
      и остаётся в `/admin/products` с бейджем «Скрыт» — проверено
      циклом скрыть→проверить→показать→проверить на реальном Товаре;
      `order_items` не затрагиваются самой природой `setProductActive()`
      (обновляет только `products.is_active`)
- [x] Фильтр по категории учитывает `product_categories` — проверено;
      диапазон цен — из активных Вариантов (условная агрегация
      `MIN/MAX(CASE WHEN is_active = 1 ...)`, а не по всем) — найдено
      расхождение с первой реализацией при сверке с этим пунктом DoD
      уже после живой проверки, исправлено и перепроверено на Товаре
      со всеми неактивными Вариантами (кол-во Вариантов не 0, цена —
      «—»)
...
**Definition of Done:**
- [x] Товар без Варианта с `is_active=1` → ошибка «добавьте Вариант с
...
- [x] Изменена цена Варианта → `/product/{slug}` показывает новую —
      проверено
- [x] `sku`, уже занятый другим Товаром → ошибка поля (`sku_taken`),
      транзакция откатана, чужой Товар не тронут — проверено; отдельно
      проверена подмена `variant_id` чужого Товара — принята как новая
      строка своего Товара, чужая не изменена
- [x] `discount_percent` = `150` → ошибка; `= 15` → сохранён (на
      витрине пока не отображается — Фаза 6) — проверено;
      `is_showroom_sample` включён → карточка показывает «Выставочный
      образец» — проверено (когда образец не самый дешёвый Вариант,
      бейдж не виден по умолчанию — это существующее поведение
      выбора Варианта по цене на витрине, не относится к этому таску)
- [x] Вариант, убранный из формы, → `is_active=0`, строка цела —
      проверено; `order_items` не проверялись отдельно (не создавался
      тестовый Заказ на тестовый Вариант — риск для чужих данных не
      оправдан, логика идентична уже проверенному `removeOrderItem()`
      Таска 4, который тоже не удаляет строку)
- [x] Две категории, основная — вторая → `is_primary` ровно у одной; —
      код-ревью (тот же `syncProductCategories()` пересобирает набор
      целиком), отдельно два-с-выбором-второй не гонялось; без основной
      → ошибка — проверено
- [x] Без JS форма с одним блоком Варианта и одной строкой
      характеристики работает — все проверки велись без JS, обычным
      POST с одним Вариантом
...
**Definition of Done:**
- [x] `.php`, переименованный в `.jpg`, отклонён по `finfo`; файл
...
- [x] Загруженное фото с `color` видно на витрине: мини-карточка
      ...
      потребовало точечной правки `product-card.php` (см. `TASK.md`/
      `dev-log.md` 19.09.2026), не в изначальном списке файлов таска
- [x] Главное фото ровно одно на Вариант после любого переключения
- [x] Удаление стирает файл с диска и строку; путь с `..` в
      подделанном запросе не выходит за `UPLOAD_PRODUCTS_DIR`
- [x] `imageId` чужого Варианта / Вариант чужого Товара → без изменений
- [x] `composer test` зелёный, включая `UploadTest` (215/215)
- [x] Проверить `.docs/dod-global.md`
...
- [x] Пресеты «сегодня/неделя/месяц» дают ожидаемые границы
      ...
      → ошибка формы, не 500
- [x] Пустой период → пустое состояние, диаграмма не падает
- [x] Суммы приходят из Model строками, `formatPrice()` только во View;
      диаграмма грузится из `public/assets/admin/`, без CDN
- [x] `composer test` зелёный, включая `ReportTest` (226/226)
- [x] Проверить `.docs/dod-global.md`
...
- [x] `.docs/phases/_status.md` — Фаза 4 → ✅ Завершена; строка
...
- [x] `.docs/tz-coverage.md` — `FR-MGR-001…004`, `FR-ADM-001, 002,
...
- [x] `.docs/admin-assembly.md` — `Q-DEV-006` помечен закрытым
