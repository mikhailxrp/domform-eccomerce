...
| 2  | Список Заказов с фильтрами и карточка Заказа (просмотр)         | ⏳ Ожидает |
| 3  | Действия над Заказом: смена статуса, оплата, доставка, отмена   | ⏳ Ожидает |
| 4  | Редактирование состава Заказа                                   | ⏳ Ожидает |
...
**Definition of Done:**
- [ ] В списке есть и Заказ, оформленный на сайте, и созданный
      напрямую в БД с другим источником контактов (гость/Покупатель) —
      оба с текущим статусом (критерий приёмки `FR-MGR-001`)
- [ ] Фильтр «Новый» показывает только `new`; неизвестное значение
      `status` в URL игнорируется, не 500
- [ ] Поиск по телефону находит и Заказ Покупателя (по `users.phone`),
      и гостевой (по `guest_phone`); ввод `8 (918) …` и `+7918…` дают
      один результат
- [ ] Карточка гостевого Заказа показывает `guest_*` и пометку «гость»;
      Заказ Покупателя — данные из `users`; несуществующий id → 404
- [ ] `customer` по прямому URL `/admin/orders` и `/admin/orders/{id}`
      → редирект, без утечки данных
- [ ] Пагинация сохраняет фильтр/поиск в ссылках страниц; весь вывод
      через `e()`, `formatPrice()`
...
**Definition of Done:**
- [ ] Добавление / изменение количества / удаление меняют
...
- [ ] В `new`, `ready_for_shipment`, `delivered`, `cancelled` блока
      нет, ручной POST → flash-ошибка без изменений
- [ ] Неактивный / несуществующий Вариант нельзя добавить; образец —
      количество 1; чужой `itemId` (другого Заказа) → без изменений
- [ ] Удаление последней позиции → ошибка «отмените Заказ»
- [ ] Снэпшот: после добавления позиции изменение цены Варианта в БД
      не меняет `order_items.price` и `total`
- [ ] Без JS: артикул + количество + «Добавить» работают обычным POST;
