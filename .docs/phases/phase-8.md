...
| 5 | Заявки на звонок в Панели управления | ✅ Завершён |
| 6 | Редактирование текстов страниц в Панели | ✅ Завершён |
| 7 | Управление баннерами слайдера | ✅ Завершён |
| 8 | Сотрудники: создание и блокировка Менеджеров (admin-only) | ✅ Завершён |
...
`admin`/`manager` → 200, `customer` → редирект, гость → `/login`;
POST без CSRF → 419; заявка с `/contacts` появляется в `new`, бейдж
работает; фильтр и откат неизвестного значения к `new` — как задумано;
25 тестовых строк → 2 страницы пагинации; несуществующий id → flash
«не найдена». Найден и исправлен до коммита баг: порядок колонок в
`SET` при идемпотентном `UPDATE` (MySQL применяет присваивания слева
направо — `processed_at` должен идти раньше `status`). Все тестовые
данные удалены после проверки. `composer test` — 375/375. Реализовано
по плану ниже без отклонений.
...
**Статус:** ✅ Завершён 22.09.2026 — проверено на реальной БД живым
HTTP (`php -S` + `curl`), тремя временными ролями (удалены после
проверки). `config/config.php`/`FileUpload.php` из плана исключены —
уже сделаны в `ADR-046`. Критерий приёмки ТЗ подтверждён напрямую:
правка текста «О компании» через Панель сразу видна на `/about`, без
деплоя (после проверки восстановлено из снапшота БД). Загрузка фото —
реальными файлами: `.php` с настоящим кодом отклонён по MIME
(`finfo`), валидный PNG сохранён и заменяет старый файл на диске,
«Удалить фото» стирает файл и `image_path`. 404 на неизвестный slug,
подсветка пустых полей, 419 без CSRF, редирект `customer`/гостя — всё
подтверждено. `composer test` — 383/383 (+8 `ContentTest`).
Реализовано по плану ниже, скорректированному при `task-init` под
уже готовые части (см. `dev-log.md`).
...
**Статус:** ✅ Завершён 22.09.2026 — проверено на реальной БД живым
HTTP (`php -S` + `curl`), тремя временными ролями (удалены после
проверки). Найден и исправлен до коммита реальный баг: файл фото
сохранялся на диск раньше проверки остальных полей формы, оставляя
осиротевшие файлы при отклонённой форме — исправлено разделением
проверки файла (`validateUpload()`) и его физического сохранения
(только после полной успешной валидации). Слайдер/toggle/замена
фото/404/CSRF — все сценарии подтверждены. `composer test` —
395/395 (+12 `BannerFormTest`). Реализовано по плану ниже без
отклонений в scope (только внутренняя доработка контроллера против
найденного бага).
...
HTTP (`php -S` + `curl`), тремя временными пользователями ролей
`admin`/`manager`/`customer` (удалены после проверки): `admin` → 200
на `/admin/users` со списком и формой; `manager` → редирект `/admin`
(без пунктов «Сотрудники»/«Настройки» в сайдбаре), `customer` →
`/account`, Гость → `/login`; POST без CSRF → 419. Создание Менеджера
через форму → строка в списке; дубликат email / пароль < 8 / роль вне
`STAFF_ROLES` — подсветка, БД не изменена. Блокировка собственной
учётки → flash-отказ, `is_blocked` не изменился. Блокировка другого
Менеджера → `is_blocked=1`, его `remember_tokens` удалены; новый вход
с тем же паролем → общий `AUTH_ERROR`; чистая сессия с одним старым
`remember_token` (после удаления токена из БД) → редирект `/login`, не
авто-вход. «Заблокировать» несуществующего id и id Покупателя
(не `manager`/`admin`) → одинаковый flash «не найден» — `setUserBlocked()`
ограничена `role IN ('manager','admin')` в самом SQL. Разблокировка
возвращает доступ. `composer test` — 408/408 (+13 `StaffFormTest`).
Реализовано по плану ниже без отклонений. Подробности —
...
- [x] Созданный Менеджер входит по выданным email/паролю и видит
      ...
      приёмки `FR-ADM-007`)
- [x] Заблокированный Менеджер: вход → общее сообщение ошибки
...
- [x] `manager`/`customer` на `/admin/users` → редирект; Гость →
      ...
      показываются
- [x] `composer test` зелёный (новые тесты `validateStaffInput()`)
- [x] Проверить `.docs/dod-global.md`
