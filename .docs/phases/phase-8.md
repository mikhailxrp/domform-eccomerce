...
🔄 В работе (1 из 8 тасков завершён)
...
| 2 | Настройки магазина: таблица `settings`, `setting()`, раздел «Настройки» (admin-only) | ⏳ Ожидает |
| 3 | «О компании» (SCR-11) и «Шоурум с картой» | ⏳ Ожидает |
| 4 | «Контакты» и форма обратного звонка | ⏳ Ожидает |
...
**Definition of Done:**
- [ ] `/about`: заголовок, текст и фото из `content_pages.about`;
      блок команды — Волков С. И. и Волкова А., демо-имён темы в HTML нет
- [ ] `/showroom`: карта загружается по URL из `settings`; при пустом
...
- [ ] `/pages/about` и `/pages/showroom` больше не отдают общий шаблон
      (редирект 301 на новые адреса или 404 — выбрать при `task-init`,
      зафиксировать в `dev-log.md`)
- [ ] В HTML нет внешних адресов, кроме `src` iframe карты и `wa.me`
