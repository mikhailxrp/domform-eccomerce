...
| # | Название | Статус |
|---|----------|--------|
| 1 | Схема БД в актуальном состоянии + сид администратора | ⏳ Ожидает |
| 2 | Базовый layout витрины и статика темы | ⏳ Ожидает |
...
**Definition of Done:**
- [ ] `install.php` выполняется дважды подряд без ошибок (идемпотентен)
- [ ] `SHOW CREATE TABLE` для каждой таблицы совпадает с `database.md`
      по колонкам, типам, FK и индексам (ручная сверка)
- [ ] `seed.php` создаёт пользователя `role='admin'` с `password_hash`
      (не открытый пароль); повторный запуск — «уже существует», без
      второй строки
- [ ] Пароль администратора нигде в репозитории — только в `.env`
...
**Definition of Done:**
- [ ] `/` открывается без ошибок в консоли браузера и в
      `storage/logs/app.log`
- [ ] В HTML нет ни одного внешнего `<link>`/`<script>` (grep по
      `http` в `header.php`/`footer.php`)
- [ ] Мобильное меню (off-canvas) темы работает — jQuery и `main.js`
      подключены корректно
- [ ] Вёрстка корректна на 320px
- [ ] Несуществующий URL → страница 404 через `ErrorHandlers`, не
      PHP-ошибка
- [ ] Проверить `.docs/dod-global.md`
