...
| 5   | Восстановление пароля (`FR-AUTH-003`)                 | ✅ Завершён |
...
против реальной БД (регистрация с ошибками валидации/дублем email, вход с
неверным паролем/несуществующим email/`is_blocked`, рейт-лимит, CSRF,
шапка меняет состояние, выход); не хватает визуальной проверки в
браузере (консоль JS, вёрстка 320px) — см. `dev-log.md` (17.09.2026)
...
- [x] Регистрация без телефона / с невалидным email / паролем короче 8
      не проходит — поля подсвечены, введённые значения сохранены в
      форме
- [x] Одно и то же сообщение для «неверный пароль», «нет такого email»
      и «email уже занят» — существование аккаунта не раскрывается
- [x] В каждом POST-обработчике `requireCsrf()` вызывается до
      обращения к Model; формы содержат `csrfField()`
- [x] После успешного входа вызван `regenerateSession()`;
      `$_SESSION['user_id']` — int
- [x] 6-я быстрая попытка входа за минуту отклоняется
      (`tooManyAttempts('login', 5, 60)`), неудачные попытки пишутся
      `logWarning()` без пароля в контексте
- [x] Пользователь с `is_blocked = 1` не входит (то же generic
      сообщение)
- [x] «Выход» завершает сессию — защищённые страницы снова редиректят
      на `/login`
- [x] `composer test` зелёный, включая `ValidationTest`
- [x] Проверить `.docs/dod-global.md`
...
**Статус:** ✅ Завершён — код реализован и проверен серией `curl`-запросов
против реальной БД (идентичный ответ для существующего/несуществующего
email, аннулирование прежнего токена при повторном запросе, одноразовость
и протухание ссылки, смена пароля со сменой `password_verify()`, рейт-лимит
`/forgot-password`, CSRF на обоих POST); не хватает визуальной проверки в
браузере — см. `dev-log.md` (17.09.2026)
...
- [x] Для любого email — одинаковый ответ «если адрес зарегистрирован,
      письмо отправлено»; письмо реально уходит только существующему
      пользователю
- [x] В БД хранится хэш токена; токен одноразовый (`used_at`), живёт
      60 минут; повторное открытие ссылки — «ссылка недействительна»
- [x] После смены пароля удалены все `remember_tokens` пользователя;
      новый пароль валидируется теми же правилами, что при регистрации
- [x] Запрос сброса ограничен по частоте (`tooManyAttempts('forgot',
…)`)
- [x] `APP_ENV=local`: ссылка сброса видна в письме/логе через тот же
      `sendMail('log', …)` → `logInfo()`, что и остальной проект (сам
      `app.log` пуст локально из-за `APP_LOG_LEVEL=error` в `.env` —
      настройка окружения, не баг, см. `dev-log.md`); `production`:
      письмо уходит через `mail()`
- [x] Обе формы — `csrfField()` + `requireCsrf()`
- [x] Проверить `.docs/dod-global.md`
...
- [x] `.docs/phases/_status.md` — Фаза 0 → ✅ Завершена
- [x] `.docs/tz-coverage.md` — статус `FR-AUTH-001…005`
