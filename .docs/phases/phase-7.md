...
| 8 | СМС: тексты событий, журнал-заглушка (без хуков) | ✅ Завершён |
| 9 | СМС-хуки на 5 событиях и блок «Уведомления» в Панели | ✅ Завершён |
...
скрипт создавал тестовые Заказы (с `guest_phone` и без телефона) и
вызывал `sendOrderSms()` напрямую — Заказ с телефоном дал строку в
`sms_notifications` и запись в `app.log` с замаскированным телефоном
(`+792****4567`), Заказ без телефона — только `WARNING` в логе, без
записи в БД. Тестовые Заказы удалены после проверки.
`php database/install.php` дважды подряд — без ошибок. `composer
test` — 337/337 (было 325, +12 `NotificationTest`). Реализовано без
отклонений от плана ниже.
...
**Статус:** ✅ Завершён 21.09.2026 — проверено живым HTTP на реальной БД
(`php -S` + `curl`): временный Менеджер и два тестовых Заказа
(ручное создание с предоплатой через Панель + гостевой чекаут),
удалены после проверки. Ручной Заказ с предоплатой сразу дал 2 строки
`sms_notifications` («принят», «подтверждён», `status=confirmed`).
Полный проход `in_production → ready_for_shipment → shipping →
delivered` довёл журнал до ровно 5 строк (на `delivered` — без
СМС), в верном порядке отражённых в блоке «Уведомления» на странице
Заказа. Запрещённый переход (`new → shipping` напрямую) — СМС не
добавлено. Повторное «Отметить предоплату» на уже подтверждённом
Заказе (`PREPAID_RESULT_ALREADY`) — без лишней строки. Гостевой Заказ
(чекаут) → «принят» с `guest_phone`; отмена из `new` → строка «Заказ
отменён» (2 строки итого, без «подтверждён» — предоплаты не было).
Искусственная ошибка внутри `sendOrderSms()` (FK на несуществующий
`order_id`) подтверждена отдельным скриптом — исключение не выходит
наружу, пишется `ERROR` в `app.log`, вызывающий код продолжает
работу. `composer test` — 337/337 (без изменений — регрессия, этот
таск не добавляет чистую логику без БД). Реализовано без отклонений
от плана ниже.
