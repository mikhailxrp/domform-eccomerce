...
| 2 | История заказов и страница Заказа | ⏳ Ожидает |
| 3 | Личные данные и смена пароля | ⏳ Ожидает |
| 4 | Книга адресов: таблица и CRUD | ⏳ Ожидает |
...
- [ ] Список содержит только Заказы с `user_id` текущего пользователя;
      подмена `id` чужого Заказа в URL → 404 (изоляция по `user_id`,
      `dod-global.md`)
- [ ] Гостевой Заказ, оформленный до регистрации на тот же email,
      виден после регистрации (регрессия `linkGuestOrdersToUser()`)
- [ ] Статусы — русские подписи и бейджи `OrderStatus.php`; статус
      оплаты и способ оплаты — подписи `PAYMENT_*_LABELS`
- [ ] Суммы через `formatPrice()`; позиции — из снэпшотов `order_items`,
      не из текущих цен Варианта
- [ ] У `delivered` показан срок гарантии (`delivered_at` + 18 мес.),
      у остальных — не показан
- [ ] Пустое состояние при отсутствии Заказов; пагинация при
...
- [ ] Добавление/редактирование/удаление → строки `addresses` с
      `user_id` текущего пользователя (критерий приёмки `FR-ACC-002`)
- [ ] Чужой `id` адреса в URL/POST → 404 или «не найден», данные не
      изменены (изоляция по `user_id`)
- [ ] Пустая форма → город/улица/дом подсвечены, остальные значения
      сохранены; 419 без CSRF
- [ ] Первый адрес автоматически основной; «сделать основным» снимает
      отметку с прежнего; удаление основного → основной назначен
      другому, среди адресов пользователя всегда ≤ 1 `is_default = 1`
- [ ] `ACCOUNT_ADDRESSES_MAX` + 1 → flash-ограничение, запись не
      создана
- [ ] `AddressTest`: `formatAddress()` с квартирой/без, с
      комментарием/без; валидатор — обязательные поля и длины
