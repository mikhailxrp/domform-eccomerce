...
| 6   | Виджет чата на витрине и предупреждение о личных данных                       | 🔄 В работе |
...
**Статус:** 🔄 В работе — код готов, структурная/функциональная
проверка пройдена без браузера (`TASK.md`, `dev-log.md` 22.09.2026);
визуальную часть DoD (320px, наложение на `.back-to-top`, XSS-текст,
консоль браузера) проверяет пользователь сам — в окружении нет
chromium-cli/Playwright.
